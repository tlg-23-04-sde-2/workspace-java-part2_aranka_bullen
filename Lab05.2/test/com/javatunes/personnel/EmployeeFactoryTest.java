...
public class EmployeeFactoryTest {
    ...
    public void testCreateEmployeeSalaried() {
        ...
        // prove that emp is exactly type Salariedemployee ( and not Executive
        assertEquals(SalariedEmployee.class, emp.getClass());

        //prove that its properties have all been set from the map
        verifiedCommonProperties(emp);

        //need to downcast reference emp to more specific temp SalariedEmployee
    ...
    private void verifiedCommonProperties(Employee emp) {
    ...
    public void CreateEmployeeHourly() {
        ...
        // prove that emp is exactly type Hourlyemplowee ( and not Executive
    ...
    public void createEmployee_shouldThrowIllegalArgumentException_whenInvalid() {
        Map<String, String> map = Map.of("type", "INVALID-TYPE");

        EmployeeFactory.createEmployee(map);  // should trigger exception
    }
}